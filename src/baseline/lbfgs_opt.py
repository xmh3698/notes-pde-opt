...
for i in range(x_sample.shape[0]):
    ...
    path = f'image/LBFGS/seed{seed}/result{i}.png'
    utils.save_pattern_sequence(y_best, path, eff_best)
    save_grad_hist(grad_norms, f'image/LBFGS/seed{seed}/grad_hist_{i}.png')
...
# Save results
np.savez(f'result/lbfgs/LBFGS_{seed}.npz',**results)
np.savez(f'result/opt_hist/LBFGS_opt_hist_{seed}.npz', opt_hist=opt_hist)
#breakpoint()
...
plt.grid(False)
plt.tight_layout()
plt.savefig(f'image/LBFGS/seed{seed}/Figure_1.png')
plt.close()
...
with open(f'image/LBFGS/seed{seed}/meta.json', 'w') as f:
